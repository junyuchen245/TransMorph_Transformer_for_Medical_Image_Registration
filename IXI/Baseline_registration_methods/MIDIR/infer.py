...
def main():
    ...
    model_idx = -1
    weights = [1, 1]
    model_folder = 'MIDIR_ncc_{}_diffusion_{}/'.format(weights[0], weights[1])
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            x_segs = []
            for i in range(46):
                def_seg = transformation.warp(x_seg_oh[:, i:i + 1, ...].float(), disp.cuda(), interp_mode='bilinear')
                x_segs.append(def_seg)
            x_segs = torch.cat(x_segs, dim=1)
            def_out = torch.argmax(x_segs, dim=1, keepdim=True)
            del x_segs, x_seg_oh
            tar = y.detach().cpu().numpy()[0, 0, :, :, :]
            jac_det = utils.jacobian_determinant_vxm(disp.detach().cpu().numpy()[0, :, :, :, :])
