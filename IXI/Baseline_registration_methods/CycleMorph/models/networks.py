...
class Dense3DSpatialTransformerNN(nn.Module):
    ...
    def _interpolate(self, input, D_upmesh, H_upmesh, W_upmesh):
        ...
        wgt_000 = (dWidth*dHeight*dDepth).unsqueeze_(1)
        wgt_100 = (dWidth * dHeight * torch.round(1-dDepth)).unsqueeze_(1)
        wgt_010 = (dWidth * torch.round(1-dHeight) * dDepth).unsqueeze_(1)
        wgt_110 = (dWidth * torch.round(1-dHeight) * torch.round(1-dDepth)).unsqueeze_(1)
        wgt_001 = (torch.round(1-dWidth) * dHeight * dDepth).unsqueeze_(1)
        wgt_101 = (torch.round(1-dWidth) * dHeight * torch.round(1-dDepth)).unsqueeze_(1)
        wgt_011 = (torch.round(1-dWidth) * torch.round(1-dHeight) * dDepth).unsqueeze_(1)
        wgt_111 = (torch.round(1-dWidth) * torch.round(1-dHeight) * torch.round(1-dDepth)).unsqueeze_(1)
...
class registUnetBlock(nn.Module):
    def __init__(self, input_nc, encoder_nc, decoder_nc):
        ...
        self.stn = Dense3DSpatialTransformer()#((160, 192, 224))
