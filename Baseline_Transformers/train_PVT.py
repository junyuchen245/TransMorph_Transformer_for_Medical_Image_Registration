...
def main():
    ...
    weights = [1, 0.02]
    save_dir = 'PVT_mse_{}_diffusion_{}/'.format(weights[0], weights[1])
    ...
    Initialize model
    '''
    config_pvt = CONFIGS_pvt['PVT-Net']
    ...
    '''
    reg_model = utils.register_model(config_pvt.img_size, 'nearest')
    reg_model.cuda()
    reg_model_bilin = utils.register_model(config_pvt.img_size, 'bilinear')
    ...
    for epoch in range(epoch_start, max_epoch):
        ...
        with torch.no_grad():
            for data in val_loader:
                ...
                y_seg = data[3]
                x_in = torch.cat((x, y), dim=1)
                grid_img = mk_grid_img(8, 1, config_pvt.img_size)
                output = model(x_in)
