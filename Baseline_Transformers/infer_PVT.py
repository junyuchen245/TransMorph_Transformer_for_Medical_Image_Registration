...
def main():
    ...
    model_idx = -1
    weights = [1, 0.02]
    model_folder = 'PVT_mse_{}_diffusion_{}/'.format(weights[0], weights[1])
    ...
    model.cuda()
    reg_model_nn = utils.register_model(config_pvt.img_size, 'nearest').cuda()
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            output = model(y_in)
            flow = output[1]
            def_out = reg_model_nn([y_seg.cuda().float(), output[1].cuda()])
