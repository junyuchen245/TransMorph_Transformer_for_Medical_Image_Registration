...
- `deformed_fixed_image.nii.gz` (Optional): If diffeomorphic registration is used, deformed fixed image in the template space using the inverse displacement field is saved.
- `displacement_field.nii.gz`: Displacement field generated by TransMorph.
- `displacement_field_inverse.nii.gz` (Optional): If diffeomorphic registration is applied, the deformed fixed image in template space, generated using the inverse displacement field, is saved.
