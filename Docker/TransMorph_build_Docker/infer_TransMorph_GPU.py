...
def main():
    ...
    wts_dir = "/app/pretrained_weights/"
    data_json = "/app/input_dataset.json"
    config_json = "/app/configs_registration.json"
    template_nib = nib.load("/app/template.nii.gz")
    ...
    if if_deformable:
        ...
        spatial_trans_tr = TransMorph.SpatialTransformer((H, W, D)).cuda()
        spatial_trans_nn = TransMorph.SpatialTransformer((H, W, D), mode='nearest').cuda()
    ...
    for img_pair in dataset_pairs:
        ...
        if verbose: print('""""""""""""""""""""""""""""""""""""""""""""\n    Step 3: Deformable Registration    \n""""""""""""""""""""""""""""""""""""""""""""\n')
        if if_deformable:
            mov_torch = torch.from_numpy(mov_npy[None, None, ...]).cuda()
            fix_torch = torch.from_numpy(fix_npy[None, None, ...]).cuda()
            x_half = F.avg_pool3d(mov_torch, 2)
            y_half = F.avg_pool3d(fix_torch, 2)
            if if_instance_optimization:
                criterion_sim = losses.NCC_vxm().cuda()
                criterion_reg = losses.Grad3d(penalty='l2').cuda()
                model.load_state_dict(pretrained['state_dict'])
