...
class PCC_SSIM(torch.nn.Module):
    def __init__(self, pcc_wt=0.5):
        ...
        self.ssim3D = SSIM3D(window_size=9)
...
class KL_MSE(torch.nn.Module):
    def __init__(self, ):
        super(KL_MSE, self).__init__()
        self.MSE_loss = nn.MSELoss()
        self.BCE_loss = nn.BCELoss()
        self.MAE_loss = nn.L1Loss()
        self.SSIM_loss = SSIM3D(window_size=9)
        self.MIND_loss = MIND_loss()
    def forward(self, net_out, y_true):
        y_pred, mu, logvar = net_out
